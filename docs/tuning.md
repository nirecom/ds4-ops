...
| `--kv-disk-space-mb` | `65536` | 64 GB cap: skips pathologically large single checkpoints and bounds eviction. Not a total-write cap (eviction deletes, so churn can still exceed it). Raised from 32 GB (2026-07-11) — the 32 GB budget stayed full and evicted checkpoints a growing session still wanted, forcing repeat full prefill; disk had 996 GB free so the cap, not the disk, was the constraint. Raised cautiously, not back to the 128 GB tied to the write-storm freeze below, since only `--kv-cache-continued-interval-tokens` (unchanged) was confirmed as the actual churn-rate lever. |
